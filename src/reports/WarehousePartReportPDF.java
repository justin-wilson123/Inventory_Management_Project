...
public class WarehousePartReportPDF extends ReportMaster {
	/**
	...
	private static Logger log = Logger.getLogger(WarehousePartReportPDF.class);
	
	public WarehousePartReportPDF(ReportGateway gw) {
		super(gw);
	...
	public void generateReport() throws ReportException {
		...
		try {
			...
			for(int i=0; i < size; i++) {
				...
				// record to trace
				String record = records.get(i).get("warehouse_name")+" | "+ records.get(i).get("part_number")+" | "+ records.get(i).get("part_name")+" | "+ records.get(i).get("quantity")+" | "+ records.get(i).get("unit_of_qty");
				log.debug(record);
				...
				content.showText( records.get(i).get("unit_of_qty") );
				content.endText();			
